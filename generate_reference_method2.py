...
@click.command()
@click.option('--input_file', required=True, help='TSV input file with probe_id, gene_name, gene_symbol, composite_group, hyb_region_lhs, hyb_region_rhs')
...
def main(input_file, output_prefix):
    ...
    with open(input_file, newline='') as infile, \
        ...
        for row in reader:
            ...
            composite_group = row['composite_group'].strip()
            ...
            if match:
                ...
                meta_writer.writerow([gene_name, sequence, composite_id, 'TRUE', 'unspliced'])
            else:
                continue
